...
def index(date0):
    ...
    for temp in cursor.fetchall():
        ...
        objs.append({'xcode':temp[0], 'xsubj':re.sub(r'\[\d{1,2}:\d{2}\]', '', temp[1]).strip(), 'xname':temp[2], 'xdesc':temp[3], 'date0':date0, 'active':active})

    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        # AJAX 요청일 경우 본문과 추가 헤더를 JSON으로 반환
        html_content = render_template('bot_v3.html', objs=objs, date0=date0, date_reform=date_reform)
        return jsonify({'html': html_content})
    else:
        return render_template('bot_v3.html', objs=objs, date0=date0, date_reform=date_reform)
